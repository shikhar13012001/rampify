...
# explicitly labeled "RAMPCUT SAMPLE" in-frame so it can never be mistaken
...
"$FF" -y -f lavfi -i "mandelbrot=size=640x360:rate=30:end_scale=400" \
  ...
  -t 6 \
  -vf "drawtext=text='RAMPCUT SAMPLE — not real footage':fontcolor=white@0.55:fontsize=16:x=16:y=h-32:box=1:boxcolor=black@0.35:boxborderw=6" \
